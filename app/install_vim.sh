...
# ビルドに必要なパッケージを指定
PKG_LIST=("make" "clang" "libtool-bin")

# makeのターゲット指定
BUILD_TARGET=

# ./configure時のオプション指定(変数を使用するときは、prefix_make_proc()に記載)
CONFIGURE_OPTIONS="--with-features=huge --enable-multibyte --enable-fail-if-missing"
##############################################
...
individual_usage()
{
  echo "  --enable-python3  enable Python3 function"
  echo "  --enable-lua      enable Lua function"
  echo "  --enable-ruby     enable Ruby function"
}
...
individual_parse_args()
{
  case $1 in
    --enable-python3 )
      PKG_LIST+=("libpython3-dev" "python3")
      CONFIGURE_OPTIONS+=" --enable-python3interp=yes"
      ;;
    --enable-lua )
      PKG_LIST+=("liblua5.2-dev" "lua5.2")
      CONFIGURE_OPTIONS+=" --enable-luainterp=yes"
      ;;
    --enable-ruby )
      PKG_LIST+=("ruby-dev")
      CONFIGURE_OPTIONS+=" --enable-rubyinterp=yes"
      ;;
    * )
      usage
      exit 0
