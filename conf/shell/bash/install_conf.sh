...
# .bashrcファイルパス
BASHRC=${CONF_ROOT_PATH}/.bashrc

# .inpurrcファイルパス
INPUTRC=${CONF_ROOT_PATH}/.inputrc

# .profileファイルパス
PROFILE=${CONF_ROOT_PATH}/.profile

# .bash_aliasesファイルパス
BASH_ALIASES=${CONF_ROOT_PATH}/.bash_aliases
##############################################
...
{
  # .bashrcがなければ設定ファイル置き場にコピー
  if [ ! -e ${BASHRC} ]; then
    cp /et/skel/.bashrc ${BASHRC}
  fi
...
function suffix_proc()
{
  ...
  if [ "`grep "#umask 022" ${PROFILE}`" != "" ]; then
    # マスク設定が無効化されている場合
    sed -i -e "s/^#umask/umask/" ${PROFILE}
  else
    # マスク設定がない場合
    insert_string_in_file ${PROFILE} "umask 022"
  fi
